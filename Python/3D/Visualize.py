...
class Visualize():
    ...
    def viz_VisualizeDICOM(self, path, option=0):
        ...
        # dc_alpha_func.AddPoint(-150, 0.0)  # 皮膚顯型
        # dc_alpha_func.AddPoint(150, 0.0)  # 皮膚顯型
        # dc_alpha_func.AddPoint(-50, 1.0)  # 皮膚顯型
        dc_alpha_func.AddPoint(-249, 0.0)  # 路線
        dc_alpha_func.AddPoint(-3024, 0.0)
        dc_alpha_func.AddPoint(78, 1.0)  # 0.29 皮膚
        dc_alpha_func.AddPoint(100, 0.55)  # 骨頭器官腎臟 原值為0.55
        dc_alpha_func.AddPoint(260, 1)  # 0.84 幾乎全身
        ...
        if option == 1:
            ...
            dc_alpha_func.AddPoint(78, 0.0)  # 0.29 皮膚
    ...
    def viz_VisualizeDICOM_augment_many(self, path):
        ...
        # dc_color_func.AddRGBPoint(-250, 0.53, 0.53, 0.53)
        dc_color_func.AddRGBPoint(2005, 0.0, 0.0, 0.0)  # 路線
        dc_color_func.AddRGBPoint(2006, 0.0, 1.0, 0.0)  # 路線4
        dc_color_func.AddRGBPoint(2007, 0.0, 0.0, 0.0)  # 路線
        dc_color_func.AddRGBPoint(2008, 0.7, 1.0, 0.7)  # 路線5
        dc_color_func.AddRGBPoint(2009, 0.0, 0.0, 0.0)  # 路線
        dc_color_func.AddRGBPoint(2010, 0.9, 1.0, 0.9)  # 路線6
    ...
    def viz_VisualizeDICOM_augment_many1(self, path):
        ...
        # dc_color_func.AddRGBPoint(-250, 0.53, 0.53, 0.53)
        dc_color_func.AddRGBPoint(2011, 0.0, 0.0, 0.0)  # 路線
        dc_color_func.AddRGBPoint(2012, 0.0, 1.0, 0.0)  # 路線7
        dc_color_func.AddRGBPoint(2013, 0.0, 0.0, 0.0)  # 路線
        dc_color_func.AddRGBPoint(2014, 0.7, 1.0, 0.7)  # 路線8
        dc_color_func.AddRGBPoint(2015, 0.0, 0.0, 0.0)  # 路線
        dc_color_func.AddRGBPoint(2016, 0.9, 1.0, 0.9)  # 路線9
...
def vis(dicom_path, dicom_path2, dicom_path3, dicom_path4, casenumber):
    global current_ren
    ...
    viz3.viz_SetViewportName(
        'Necrotic pancreas and Skin ( Fluid area )', 10, 30)
    ...
    def KeyPressEvent(obj, event):
        ...
        if keyCode in ['p', 'P']:
            if PRINT_PATH == 0:
                ...
                viz2.viz_SetViewportName('R > G > B', 10, 200)
        ...
        if keyCode in ['o', 'O']:
            if PRINT_PATH == 0:
                ...
                viz2.viz_SetViewportName('Light > Dark', 10, 200)
                PRINT_PATH = 1
            else:
                viz2.GetRenderer().RemoveVolume(viz2.GetPath())
                viz3.GetRenderer().RemoveVolume(viz3.GetPath())
        ...
        if keyCode in ['u', 'U']:
            if PRINT_PATH == 0:
                viz2.viz_VisualizeDICOM_augment(dicom_path4)
                viz2.viz_VisualizeDICOM_augment(dicom_path4)
                viz2.viz_SetViewportName('AstarLine', 10, 260)
                viz2.viz_SetViewportName(' R >  G  > B ', 10, 200)
                PRINT_PATH = 1
            else:
                viz2.GetRenderer().RemoveVolume(viz2.GetPath())
        ...
        if keyCode in ['y', 'Y']:
            if PRINT_PATH == 0:
                viz2.viz_VisualizeDICOM_augment_many(dicom_path4)
                viz3.viz_VisualizeDICOM_augment_many(dicom_path4)
                viz2.viz_SetViewportName('AstarLine', 10, 260)
                PRINT_PATH = 1
            else:
                viz2.GetRenderer().RemoveVolume(viz2.GetPath())
